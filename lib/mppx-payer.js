...
export const MPP_CLIENT_ID = "pact-client";
...
export const FILE_KEYSTORE_ACK = "container-operator";

/**
 * Headless container operators have no OS keychain. With an explicit ack they
 * may keep each account key in its own strictly-validated 0600 file instead.
 * This never widens the default posture: the raw environment-key path stays
 * disabled, the ack must match exactly, and every check mirrors the identity
 * config rules (regular non-symlink file, current-user owned, mode 0600).
 */
export async function resolveFileKeystoreAccount(name) {
  if (process.env.PACT_MPPX_ALLOW_FILE_KEYSTORE?.trim() !== FILE_KEYSTORE_ACK) return null;
  const dir = process.env.PACT_MPPX_KEYSTORE_DIR?.trim();
  if (!dir) {
    throw new Error("PACT_MPPX_KEYSTORE_DIR is required when PACT_MPPX_ALLOW_FILE_KEYSTORE is set");
  }
  const accountName = (name ?? process.env.MPPX_ACCOUNT ?? "").trim();
  if (!/^[A-Za-z0-9._-]{1,64}$/.test(accountName)) {
    throw new Error("file keystore requires an --account (or MPPX_ACCOUNT) matching [A-Za-z0-9._-]{1,64}");
  }
  const path = join(dir, `${accountName}.key`);
  const stat = lstatSync(path, { bigint: true, throwIfNoEntry: false });
  if (!stat) throw new Error(`file keystore account not found: ${path}`);
  if (stat.isSymbolicLink() || !stat.isFile()) {
    throw new Error(`keystore key must be a regular non-symlink file: ${path}`);
  }
  if (Number(stat.uid) !== process.getuid()) {
    throw new Error(`keystore key must be owned by the current user: ${path}`);
  }
  if (Number(stat.mode & 0o777n) !== 0o600) {
    throw new Error(`keystore key must have mode 0600: ${path}`);
  }
  const key = readFileSync(path, "utf8").trim();
  if (!/^0x[0-9a-fA-F]{64}$/.test(key)) {
    throw new Error(`keystore key must be a 0x-prefixed 32-byte hex key: ${path}`);
  }
...
export async function resolveMppxKeychainAccount(name, resolver = resolveMppxAccount) {
  ...
  const account = (await resolveFileKeystoreAccount(name)) ?? await resolver(name);
