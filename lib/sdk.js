...
export class PactClient {
  ...
  /** 402 flow: request without proof, receive a requirement, then retry with X-PAYMENT. Mock proof is automatic. */
    ...
    for (let i = 0; i < (opts.retries ?? 10); i++) {
      ...
      if (suppliedPayment !== undefined) {
        const paid = await this.http(
          "POST",
          `/pacts/${pactId}/fund`,
          this.envelope("pacts.fund", call, pactId, n),
          { "x-payment": suppliedPayment }
        );
        if (!isRetryable409(paid)) return paid;
        continue;
      }
      const first = await this.http("POST", `/pacts/${pactId}/fund`, this.envelope("pacts.fund", call, pactId, n));
      if (first.status !== 402) {
        if (isRetryable409(first)) continue;
        return first;
      }
      const requirement = first.body.requirement;
      const proof = await (opts.pay ? opts.pay(requirement) : { scheme: "mock" });
      const xp = Buffer.from(JSON.stringify(proof)).toString("base64");
      const paid = await this.http("POST", `/pacts/${pactId}/fund`, this.envelope("pacts.fund", call, pactId, n), {
        "x-payment": xp
      });
      if (!isRetryable409(paid)) return paid;
    }
