...
export class PactClient {
  ...
  async wallet(query = {}) {
    const path = `/wallets/${encodeURIComponent(this.partyId)}/query`;
    return this.http("POST", path, this.envelope("wallets.read", query, "", 0));
  }

  async prepareWalletDeposit(rail, asset, amount) {
    const call = { rail, asset, amount: String(amount) };
    const body = this.envelope("wallets.deposit", call, "", 0);
    return {
      url: `${this.server}/wallets/${encodeURIComponent(this.partyId)}/deposit`,
      method: "POST",
      body,
  ...
  async depositWallet(rail, asset, amount, opts = {}) {
    const request = await this.prepareWalletDeposit(rail, asset, amount);
    const path = `/wallets/${encodeURIComponent(this.partyId)}/deposit`;
    const first = await this.http(request.method, path, request.body, request.headers);
    if (first.status !== 402 || !first.body?.requirement) return first;
    if (!opts.pay && rail !== "mock") return first;
    const payment = await (opts.pay
      ? opts.pay(first.body.requirement, {
          body: request.body,
      ...
      : { scheme: "mock" });
    const paidHeaders = payment?.headers ?? {
      "x-payment": Buffer.from(JSON.stringify(payment)).toString("base64")
    };
    return this.http(request.method, path, request.body, paidHeaders);
  }

  async withdrawWallet(withdrawalId, rail, asset, amount) {
    const path = `/wallets/${encodeURIComponent(this.partyId)}/withdraw`;
    const call = { withdrawalId, rail, asset, amount: String(amount) };
    return this.http("POST", path, this.envelope("wallets.withdraw", call, withdrawalId, 0));
  }
  ...
  /**
   * Complete the 402 flow with one immutable SignedCall. Standard x402/MPP
   * callbacks return {headers}; local mock and explicit operator-recovery paths
   * may return a proof object, which is encoded as X-PAYMENT.
   */
  ...
  async bindRailAddress(rail, address) {
    return this.http(
      "POST",
      `/rails/${rail}/address`,
      this.envelope("rails.bindAddress", { rail, address }, "", 0)
    );
  }
  ...
  async acceptOffer(offerId, opts = {}) {
    const acceptanceId = opts.acceptanceId ?? `a_${randomUUID()}`;
    try {
      const response = await this.http(
        "POST",
        `/offers/${encodeURIComponent(offerId)}/accept`,
        this.envelope("offers.accept", { acceptanceId }, offerId, 0)
      );
      return { ...response, acceptanceId };
    } catch (error) {
      throw new Error(
        `Offer acceptance ${acceptanceId} has an uncertain response. Retry with the same acceptanceId; do not create a new purchase. ${error?.message ?? error}`,
        { cause: error }
      );
    }
  }
